...
depth_out=depth.pop(LOO)
GR_out=GR.pop(LOO)
Rt_out=Rt.pop(LOO)
density_out=density.pop(LOO)
porosity_out=porosity.pop(LOO)
Vp_out=Vp.pop(LOO)
Sgh_out=Sgh.pop(LOO)
morph_out=morph.pop(LOO)
...
X.append([])
for y in range(len(depth_out)):
    X[-1].append([GR_out[y],Rt_out[y],density_out[y],porosity_out[y],Vp_out[y]])
...
for x in range(runs):
    ...
    for y in range(len(X_norm)):
        ...
        well_scores[-1].append(sklearn.metrics.r2_score(Sgh_out,well_preds[y]))
